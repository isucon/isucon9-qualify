...
func findItemFromUsersTransactionsAll(ctx context.Context, s *session.Session, targetItemID, nextItemID, nextCreatedAt, loop, maxPage int64) (session.ItemDetail, error) {
	...
	for _, item := range items {
		...
		if item.Seller == nil {
			return session.ItemDetail{}, failure.New(fails.ErrApplication, failure.Messagef("/users/transactions.json の商品の出品者情報が返っていません (item_id: %d, user_id: %d)", item.ID, s.UserID))
		}
