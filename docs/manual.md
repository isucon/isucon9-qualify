...
**［重要］競技に利用できる計算機資源は主催者側が指定した3台のインスタンスのみです。外部のメトリクス計測サービスの使用のみ特例として許可しますが、スコアを向上させるいかなる効果も持つものであってはいけません。**
...
新着一覧については、上記の制限を満たした上でよりユーザにあわせた商品の一覧を返すことができます。
...
- 以下に示すような仕様に明記されている要件を満たせない改変を行ってはなりません
  - パスワードを平文で保存する
